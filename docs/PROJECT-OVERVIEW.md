...
1. **Регистрация** — email, имя, пароль → создаётся пользователь с `emailVerified: false` → на почту уходит **6-значный код** (если задан `RESEND_API_KEY`; в dev без ключа верификация может быть пропущена — см. код `register` route).
2. **Подтверждение** — страница `/verify-email`, API `verify-email` / `resend-verification`.
